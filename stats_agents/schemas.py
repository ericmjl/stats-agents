...
class ExperimentDescription(BaseModel):
    ...
    def get_nuisance_factors(self) -> Dict[str, List[str]]:
        ...
        Nuisance factors are sources of variation we need to control for
        but are not of primary interest (e.g., plate effects, day effects,
        operator effects, batch effects).
        """
        nuisance = {}
        for factor in self.factors:
            if factor.factor_type == FactorType.NUISANCE:
                nuisance[factor.name] = factor.levels
        return nuisance

    def get_blocking_factors(self) -> Dict[str, List[str]]:
        """Get blocking factors and their levels.

        Blocking factors are used for experimental design stratification
        to reduce confounding (e.g., experimental blocks, strata).
        """
        blocking = {}
        for factor in self.factors:
            if factor.factor_type == FactorType.BLOCKING:
                blocking[factor.name] = factor.levels
        return blocking
    ...
    def get_covariates(self) -> List[str]:
        ...
        covariates = []
        for factor in self.factors:
            if factor.factor_type == FactorType.COVARIATE or factor.is_continuous:
                covariates.append(factor.name)
        return covariates
...
def create_sample_dataframe(
    ...
    """Create a sample DataFrame based on the experiment description."""
    import numpy as np

    data = {}

    # Generate experimental unit IDs
    unit_name = experiment.units or "subject"
    data[unit_name] = [f"{unit_name}_{i + 1}" for i in range(n_samples)]
    ...
    for factor in experiment.factors:
        if factor.is_continuous:
            ...
            data[factor.name] = np.random.uniform(min_val, max_val, n_samples)
        else:
            data[factor.name] = np.random.choice(factor.levels, n_samples)

    # Generate timepoints if present
    if experiment.timepoints:
        data["timepoint"] = np.random.choice(experiment.timepoints, n_samples)

    # Generate response variable
    if experiment.response_type == ResponseType.POISSON:
        data[experiment.response] = np.random.poisson(5, n_samples)
    elif experiment.response_type == ResponseType.BINOMIAL:
        data[experiment.response] = np.random.choice([0, 1], n_samples)
    else:
        data[experiment.response] = np.random.normal(0, 1, n_samples)
